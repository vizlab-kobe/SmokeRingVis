...
This is the baseline MPI in-situ visualization application for the SmokeRing
simulation.

## Placement

```sh
SmokeRingVis/App/InSituVis.mpi/
```

The binary name is `InSituVis.mpi` because `kvsmake -G` uses the current
...
This stops a running `InSituVis.mpi` process when present, removes KVS build
files, removes `Output`, and removes the data directory specified in
`params.namelist`.
