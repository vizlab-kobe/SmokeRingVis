...
# スクリプト
POSTPROC_SCRIPT="/home/tomoya/Work/GitHub/SmokeRingVis/App/InSituVis.mpi/for_4dgs/move_point_and_viewpoint.py"
...
if [ ! -f "$POSTPROC_SCRIPT" ]; then
  echo "[ERR] 後処理スクリプトが見つかりません: $POSTPROC_SCRIPT" >&2
  exit 1
fi
...
# ===== 1) points/viewpoints/img の整形 =====
echo "[RUN] 整形: move_point_and_viewpoint -> {img,params,points}"
echo "      $PYTHON_BIN $POSTPROC_SCRIPT ${NAME}"
"$PYTHON_BIN" "$POSTPROC_SCRIPT" "${NAME}"

# ===== 2) MultipleView 形式に変換 =====
echo "[RUN] ${PY_SCRIPT}"
