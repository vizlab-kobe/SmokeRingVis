# InSituVis.mpi.CameraFocus

This MPI application visualizes the SmokeRing simulation with camera focus
control.

## Placement

```sh
SmokeRingVis/App/InSituVis.mpi.CameraFocus/
```

The binary name is `InSituVis.mpi.CameraFocus` because `kvsmake -G` uses the
current directory name as the target name.
